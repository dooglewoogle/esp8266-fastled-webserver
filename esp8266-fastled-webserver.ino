...
<<<<<<< HEAD
  if (powerIsTransitioning){
      ...
      if (timeDelta >= transitionMaxMillis){
        powerIsTransitioning = false;
      } else {
        uint8_t tempBrightness;
        uint8_t tdToBase255;
        
        if (power == 1){
          tdToBase255 = ((float)timeDelta / (float)transitionMaxMillis) * 255;
          tempBrightness = ( (float) brightness / 255) * tdToBase255;
        } else {
            tdToBase255 = ((float)timeDelta / (float)transitionMaxMillis) * 255;
            tempBrightness = brightness - (( (float) brightness / 255) * tdToBase255);
        }
            
        if (tempBrightness > 255){ tempBrightness = 255;}
        else if (tempBrightness < 0){tempBrightness = 0;};
...
void bed()
{
  ...
  CRGB blu = CRGB::RoyalBlue;
  
  for (int i=0;i<NUM_LEDS;i++){
      
      
    
    uint8_t blendAmt = (i/NUM_LEDS)*255;
    leds[i] = blend(orange, blu, blendAmt);
    };
...
uint8_t centiToUint8(uint8_t input){
    return (uint8_t) ((float) input / 100) * 255;
  }

uint8_t remapUint8ToCenti(uint8_t input){
    return (uint8_t) ((float) input / 255) * 100;
  }
